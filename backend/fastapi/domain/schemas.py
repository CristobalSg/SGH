...
class BaseSecureValidator:
    ...
    def validate_alphanumeric_code(cls, value: str, field_name: str = "código") -> str:
        ...
        if not re.match(r"^[A-Z0-9\-]+$", value):
            raise ValueError(
                f"El {field_name} solo puede contener letras (A-Z), "
                f"números (0-9) y guiones (-)"
            )
    ...
    def validate_name(cls, value: str, field_name: str = "nombre") -> str:
        ...
        if not cls.NAME_PATTERN.match(value):
            raise ValueError(
                f"El {field_name} solo puede contener letras y espacios"
            )
    ...
    @classmethod
    def validate_description(cls, value: Optional[str], field_name: str = "descripción") -> Optional[str]:
        """
    ...
    @classmethod
    def validate_range(cls, value: int, min_val: int, max_val: int, field_name: str = "valor") -> int:
        """
        ...
        if not min_val <= value <= max_val:
            raise ValueError(
                f"El {field_name} debe estar entre {min_val} y {max_val}"
            )
...
class UserSecureCreate(UserSecureBase):
    ...
    def validate_password_strength(cls, v: str) -> str:
        ...
        sequences = ['abc', 'bcd', 'cde', 'def', '123', '234', '345', '456', 'qwe', 'wer', 'ert', 'asd', 'sdf', 'dfg', 'zxc', 'xcv', 'cvb']
        v_lower = v.lower()
        has_sequence = False
...
class AsignaturaSecureBase(BaseModel, CodigoSeguroMixin):
    ...
    nombre: constr(
        ...
        examples=["Matemáticas I", "Física II"]
    )
    
    creditos: conint(ge=1, le=20) = Field(
        ...,
        description="Número de créditos (1-20)",
        examples=[3, 4, 5]
    )
...
class SeccionSecureBase(BaseModel, CodigoSeguroMixin):
    ...
    codigo: constr(
        ...
        examples=["SEC-001", "A", "B1"]
    )
    
    anio: conint(ge=2020, le=2030) = Field(
        ...,
        description="Año académico (2020-2030)",
        examples=[2024, 2025]
    )
    
    semestre: Literal[1, 2] = Field(
        ...,
    ...
    cupos: conint(ge=1, le=100) = Field(
        ...,
        description="Número de cupos disponibles (1-100)",
        examples=[30, 40, 50]
    )
...
class SalaSecureBase(BaseModel, CodigoSeguroMixin):
    ...
    codigo: constr(
        ...
        examples=["A-101", "LAB-201", "AUD-PRINCIPAL"]
    )
    
    capacidad: conint(ge=1, le=500) = Field(
        ...,
        description="Capacidad de la sala (1-500)",
        examples=[30, 50, 100]
    )
    
    tipo: TipoSalaEnum = Field(
        ...,
    ...
    disponible: bool = Field(
        default=True,
        description="Indica si la sala está disponible"
    )
    
    equipamiento: Optional[constr(max_length=500)] = Field(
        None,
        description="Descripción del equipamiento de la sala"
    )
...
class BloqueSecureBase(BaseModel, HorarioSecureMixin):
    ...
    hora_inicio: time = Field(
        ...
        examples=["08:00:00", "14:30:00"]
    )
    
    hora_fin: time = Field(
        ...,
        description="Hora de fin del bloque",
        examples=["09:30:00", "16:00:00"]
    )
    ...
    def validate_business_hours(cls, v: time) -> time:
        ...
        if not (time(8, 0) <= v <= time(21, 0)):
            raise ValueError(
                'Las horas deben estar entre 08:00 y 21:00'
            )
        return v
...
class RestriccionHorarioSecureBase(BaseModel, HorarioSecureMixin):
    ...
    activa: bool = Field(
        default=True,
        description="Indica si la restricción está activa"
    )
...
class RestriccionSecureBase(BaseModel, BaseSecureValidator):
    ...
    tipo: TipoRestriccionEnum = Field(
        ...,
    ...
    prioridad: conint(ge=1, le=10) = Field(
        ...,
        description="Prioridad de la restricción (1-10)",
        examples=[1, 5, 10]
    )
    
    restriccion_blanda: bool = Field(
        default=False,
        description="Indica si es una restricción blanda (puede violarse)"
    )
    
    restriccion_dura: bool = Field(
        default=False,
        description="Indica si es una restricción dura (no puede violarse)"
    )
    ...
    def validate_restriccion_type(self):
        ...
        if not self.restriccion_blanda and not self.restriccion_dura:
            raise ValueError(
                'La restricción debe ser blanda o dura (o ambas)'
            )
        return self
...
class CampusSecureBase(BaseModel, BaseSecureValidator):
    ...
    nombre: constr(
        ...
        examples=["Campus Central", "Campus Norte"]
    )
    
    direccion: Optional[constr(max_length=200)] = Field(
        None,
...
class EdificioSecureBase(BaseModel, BaseSecureValidator):
    ...
    nombre: constr(
        ...
        examples=["Edificio A", "Torre Norte", "Pabellón 1"]
    )
    
    pisos: Optional[conint(ge=1, le=50)] = Field(
        None,
        description="Número de pisos del edificio (1-50)"
    )
...
class SeccionSecureCreate(SeccionSecureBase, IDPositivoMixin):
    ...
    asignatura_id: conint(gt=0) = Field(
        ...,
...
class EdificioSecureCreate(EdificioSecureBase, IDPositivoMixin):
    ...
    campus_id: conint(gt=0) = Field(
        ...,
        description="ID del campus asociado"
    )
...
class ClaseSecureCreate(ClaseSecureBase, IDPositivoMixin):
    ...
    sala_id: conint(gt=0) = Field(
        ...,
        description="ID de la sala"
    )
    
    bloque_id: conint(gt=0) = Field(
        ...,
...
class PaginationParams(BaseModel):
    ...
    skip: conint(ge=0, le=10000) = Field(
        default=0,
...
for schema_class in [
    UserSecureBase, UserSecureCreate, UserSecureLogin,
    AsignaturaSecureBase, AsignaturaSecureCreate,
    SeccionSecureBase, SeccionSecureCreate,
    SalaSecureBase, SalaSecureCreate,
    BloqueSecureBase, BloqueSecureCreate,
    RestriccionSecureBase, RestriccionSecureCreate,
    RestriccionHorarioSecureBase, RestriccionHorarioSecureCreate,
