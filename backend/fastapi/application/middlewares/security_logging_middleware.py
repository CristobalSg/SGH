...
class SecurityLoggingMiddleware(BaseHTTPMiddleware):
    ...
    SENSITIVE_ENDPOINTS = {
        "/api/auth/login", "/api/auth/login-json",
        "/api/auth/register", "/api/auth/refresh",
        "/api/users", "/api/administradores"
    }
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            if request.query_params:
                log_data["query_params"] = self._redact_sensitive_data(
    ...
    def _log_security_events(
        ...
        if path in ["/api/auth/login", "/api/auth/login-json"] and status_code == 200:
            logger.info(
                f"SECURITY EVENT: Successful login from {client_ip} "
        ...
        # Registro de nuevo usuario
            ...
            logger.info(
                f"SECURITY EVENT: New user registration from {client_ip}"
            )
        
        # Intentos de acceso no autorizado
        elif status_code == 403:
            logger.warning(
                f"SECURITY EVENT: Forbidden access attempt to {path} from {client_ip}"
            )
        ...
        # Múltiples errores 4xx (posible ataque)
            ...
            logger.info(
                f"SECURITY EVENT: Client error {status_code} on {path} from {client_ip}"
            )
