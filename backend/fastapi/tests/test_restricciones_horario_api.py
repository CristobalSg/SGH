...
class TestRestriccionesHorarioAdminEndpoints:
    ...
    def test_admin_create_restriccion_horario_success(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        response = client.post("/api/restricciones-horario/", json=restriccion_data, headers=auth_headers_admin)
    ...
    def test_admin_create_restriccion_horario_invalid_hours(self, client: TestClient, auth_headers_admin, docente_completo):
    ...
    def test_admin_create_restriccion_horario_invalid_day(self, client: TestClient, auth_headers_admin, docente_completo):
    ...
    def test_admin_get_restricciones_horario(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        client.post("/api/restricciones-horario/", json=restriccion_data, headers=auth_headers_admin)
    ...
    def test_admin_get_restricciones_horario_with_pagination(self, client: TestClient, auth_headers_admin, docente_completo):
        """Test paginación en obtener restricciones de horario"""
        response = client.get("/api/restricciones-horario/?skip=0&limit=10", headers=auth_headers_admin)
    ...
    def test_admin_get_restriccion_horario_by_id(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        create_response = client.post("/api/restricciones-horario/", json=restriccion_data, headers=auth_headers_admin)
        created_id = create_response.json()["id"]
        
        # Obtener por ID
        response = client.get(f"/api/restricciones-horario/{created_id}", headers=auth_headers_admin)
    ...
    def test_admin_get_restriccion_horario_not_found(self, client: TestClient, auth_headers_admin, docente_completo):
    ...
    def test_admin_update_restriccion_horario_patch(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        response = client.patch(f"/api/restricciones-horario/{created_id}", json=patch_data, headers=auth_headers_admin)
    ...
    def test_admin_update_restriccion_horario_empty_data(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        response = client.patch(f"/api/restricciones-horario/{created_id}", json={}, headers=auth_headers_admin)
        assert response.status_code == 400

    def test_admin_delete_restriccion_horario(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        # Eliminar
        response = client.delete(f"/api/restricciones-horario/{created_id}", headers=auth_headers_admin)
        
        assert response.status_code == 204
        
        # Verificar que ya no existe
        get_response = client.get(f"/api/restricciones-horario/{created_id}", headers=auth_headers_admin)
        assert get_response.status_code == 404

    def test_admin_get_restricciones_by_docente(self, client: TestClient, auth_headers_admin, docente_completo):
    ...
    def test_admin_get_restricciones_by_day(self, client: TestClient, auth_headers_admin, docente_completo):
    ...
    def test_admin_get_disponibilidad_docente(self, client: TestClient, auth_headers_admin, docente_completo):
        ...
        response = client.get("/api/restricciones-horario/disponibilidad/1", headers=auth_headers_admin)
        
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)

    def test_admin_get_disponibilidad_docente_with_day(self, client: TestClient, auth_headers_admin, docente_completo):
        """Test obtener disponibilidad de un docente para día específico"""
        response = client.get("/api/restricciones-horario/disponibilidad/1?dia_semana=1", headers=auth_headers_admin)
        
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)

    def test_admin_delete_restricciones_by_docente(self, client: TestClient, auth_headers_admin, docente_completo):
...
class TestRestriccionesHorarioDocenteEndpoints:
    ...
    def test_docente_get_mis_restricciones(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        ...
        create_response = client.post("/api/restricciones-horario/docente/mis-restricciones", json=restriccion_data, headers=auth_headers_docente_completo)
        assert create_response.status_code == 201
        
        # Ahora obtener todas las restricciones
        response = client.get("/api/restricciones-horario/docente/mis-restricciones", headers=auth_headers_docente_completo)
    ...
    def test_docente_create_restriccion_horario(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        """Test docente crea su propia restricción de horario"""
        restriccion_data = {
            "docente_id": docente_completo["docente_id"],  # Este valor será sobrescrito por el sistema
            "dia_semana": 1,
            "hora_inicio": "08:00",
            "hora_fin": "12:00",
            "disponible": True,
        ...
        response = client.post("/api/restricciones-horario/docente/mis-restricciones", json=restriccion_data, headers=auth_headers_docente_completo)
    ...
    def test_docente_get_restriccion_by_id(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        ...
        # Obtener por ID
        response = client.get(f"/api/restricciones-horario/docente/mis-restricciones/{created_id}", headers=auth_headers_docente_completo)
        
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == created_id

    def test_docente_update_restriccion_horario(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        ...
        # Actualizar
        patch_data = {
            "descripcion": "Actualizado por docente",
            "disponible": False
        }
        response = client.patch(f"/api/restricciones-horario/docente/mis-restricciones/{created_id}", json=patch_data, headers=auth_headers_docente_completo)
    ...
    def test_docente_delete_restriccion_horario(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        ...
        # Eliminar
        response = client.delete(f"/api/restricciones-horario/docente/mis-restricciones/{created_id}", headers=auth_headers_docente_completo)
        
        assert response.status_code == 204

    def test_docente_get_mi_disponibilidad(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        ...
        client.post("/api/restricciones-horario/docente/mis-restricciones", json=restriccion_data, headers=auth_headers_docente_completo)
        
        # Obtener disponibilidad
        response = client.get("/api/restricciones-horario/docente/mi-disponibilidad", headers=auth_headers_docente_completo)
    ...
    def test_docente_get_mi_disponibilidad_by_day(self, client: TestClient, docente_completo, auth_headers_docente_completo):
        ...
        response = client.get("/api/restricciones-horario/docente/mi-disponibilidad?dia_semana=2", headers=auth_headers_docente_completo)
    ...
    def test_docente_endpoints_unauthorized_estudiante(self, client: TestClient, auth_headers_estudiante):
        """Test que los endpoints de docente no son accesibles para estudiantes"""
        response = client.get("/api/restricciones-horario/docente/mis-restricciones", headers=auth_headers_estudiante)
        assert response.status_code == 403
...
class TestRestriccionesHorarioValidation:
    ...
    def test_descripcion_vacia_se_convierte_a_null(self, client: TestClient, auth_headers_admin, docente_completo):
