...
class TestBrokenAccessControl:
    ...
    def test_estudiante_cannot_access_admin_endpoints(self, client: TestClient, auth_headers_estudiante):
    ...
    def test_docente_cannot_delete_campus(self, client: TestClient, auth_headers_admin, 
                                         auth_headers_docente, campus_completo):
        """Docente no debe poder eliminar campus"""
        campus_id = campus_completo["id"]
        
        response = client.delete(
            f"/api/campus/{campus_id}",
            headers=auth_headers_docente
        )
        assert response.status_code == 403
    ...
    def test_horizontal_privilege_escalation_prevention(self, client: TestClient, 
                                                       auth_headers_admin):
        ...
        user1_data = {
            "nombre": "Estudiante 1",
            "email": "est1@test.com",
            "contrasena": "Estud1ant3_1!2024#Pass",
            "rol": "estudiante"
        }
        user2_data = {
            "nombre": "Estudiante 2",
            "email": "est2@test.com",
            "contrasena": "Estud1ant3_2!2024#Pass",
            "rol": "estudiante"
        }
        
        user1_response = client.post("/api/auth/register", json=user1_data, headers=auth_headers_admin)
        user2_response = client.post("/api/auth/register", json=user2_data, headers=auth_headers_admin)
        ...
        login_response = client.post("/api/auth/login", json={
            ...
            "contrasena": user2_data["contrasena"]
        })
...
class TestRateLimiting:
    ...
    def test_rate_limit_on_login_endpoint(self, client: TestClient):
        ...
        assert any(code in [401, 422] for code in responses), "Login endpoint no está respondiendo correctamente"
...
class TestRoleBasedAccess:
    ...
    def test_only_admin_can_create_campus(self, client: TestClient, auth_headers_admin, 
                                          auth_headers_docente, auth_headers_estudiante):
    ...
    def test_estudiante_cannot_modify_other_students(self, client: TestClient, 
                                                     auth_headers_admin):
        ...
        login_response = client.post(
            "/api/auth/login",
            json={
                "email": est1_data["email"],
                "contrasena": est1_data["contrasena"]
            }
        )
