...
class TestEdificiosEndpoints:
    ...
    def test_get_edificio_by_id_success(self, client: TestClient, auth_headers_admin):
        ...
        # Crear edificio
        edificio_data = {
            "nombre": "Edificio Test ID",
            "pisos": 2,
            "campus_id": campus_id
        }
        
        create_response = client.post("/api/edificios/", json=edificio_data, headers=auth_headers_admin)
    ...
    def test_update_edificio_success(self, client: TestClient, auth_headers_admin):
        ...
        response = client.put(f"/api/edificios/{created_id}", json=update_data, headers=auth_headers_admin)
    ...
    def test_campus_edificios_relationship(self, client: TestClient, auth_headers_admin):
        ...
        edificio_data = {
            "nombre": "Edificio Campus Test",
            "pisos": 4,
            "campus_id": campus_id
        }
        
        edificio_response = client.post("/api/edificios/", json=edificio_data, headers=auth_headers_admin)
...
class TestSeccionesEndpoints:
    ...
    def test_create_seccion_success(self, client: TestClient, auth_headers_admin):
        ...
        asignatura_response = client.post("/api/asignaturas/", json=asignatura_data, headers=auth_headers_admin)
...
class TestClasesEndpoints:
    ...
    def test_create_clase_success(self, client: TestClient, auth_headers_admin):
        ...
        if docente_response.status_code != 201:
            ...
            docente_id = docente_data_response.get("id") or docente_data_response.get("user_id") or 2
        ...
        seccion_response = client.post("/api/secciones/", json=seccion_data, headers=auth_headers_admin)
